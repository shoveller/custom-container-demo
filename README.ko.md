...
custom-container-demo/
  src/
    index.ts                 # Worker와 Container 라우팅
  container_src/
...
- `GET /` — 예제 route 목록
- `GET /singleton` — 단일 container instance로 요청 전달
- `GET /container/<id>` — 이름 있는 container instance로 요청 전달
- `GET /lb` — 여러 container instance 중 하나로 요청 전달
